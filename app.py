...
video_files = glob.glob('/Users/poojapullakhandam/Downloads/archive/Celeb-real/*.mp4')
video_files += glob.glob('/Users/poojapullakhandam/Downloads/archive/Celeb-synthesis/*.mp4')
video_files += glob.glob('/Users/poojapullakhandam/Downloads/archive/YouTube-real/*.mp4')
x=int(len(video_files)*0.8)
train_videos=video_files[:x]   
test_videos=video_files[x:]
...
def find_label(csv_file, file_name):
    with open(csv_file, newline='') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            if row['label'] == file_name:
                return row['file']
    return None
def number_of_real_and_fake_videos(data_list):
    header_list = ["label","file"]
    lab = pd.read_csv('/Users/poojapullakhandam/Downloads/archive/List_of_testing_videos.csv',names=header_list)
    l=[]
    for temp_video in data_list:
        temp_video=list(temp_video.split('/'))
        temp_video=temp_video[-2]+'/'+temp_video[-1]
        matching_rows = lab[lab['label'] == temp_video]
        if not matching_rows.empty:
            label = matching_rows.iloc[0]['file']
            if label == '0':
                l.append(0)
            else:
                l.append(1)
    ...
    r=recall_score(y_true,y_pred,average='micro')*100
    print("Recall=  ",r)
    p=precision_score(y_true,y_pred,average='micro')*100
    print("Precision=  ",p)
